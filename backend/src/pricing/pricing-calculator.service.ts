  ...
  async calculateCartPricing(cartId: string) {
    ...
    for (const item of cart.items) {
      ...
      await this.prisma.cartItem.update({
        where: { id: item.id },
        data: {
          listPrice: new Decimal(price.listPrice),
          unitPrice: new Decimal(price.companyPrice),
          discountAmount: new Decimal(price.discount),
    ...
    await this.prisma.cart.update({
      ...
      data: {
        ...
        total: new Decimal(subtotal), // Tax will be calculated separately
